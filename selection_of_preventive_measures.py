...
class App(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.title = 'Персонифицированный подбор превентивных мероприятий на прегравидарном этапе с учетом \
стратификации женщин по риску преэклампсии'
        self.left = 100
        self.top = 100
        self.width = 1250
        self.height = 900
    ...
    def calc_prediabetes(self):
        try:
            ...
            if self.combobox5.currentText() == 'Нет':
                ...
                counter_prediabetes += 5
            # вывод результата
            if (counter_prediabetes < 12) and (self.combobox6.currentText() == 'Нет') and (
                    float(self.textbox5.text()) < 6.0):
    ...
    def update_text(self):
        rec_main = """Рекомендовано:
Персонифицированные превентивные мероприятия на догестационном этапе:
"""
        rec_met = """Прием Метформина. Суточная доза – 1000 мг после или во время приема пищи, разделенная на 2 приема \
(при отсутствии противопоказаний);
"""
        rec_lifestyle = """Модификация образа жизни:
- увеличение физической активности умеренной интенсивности (быстрая ходьба, плавание, велосипед, танцы – \
продолжительностью 30-60 минут предпочтительно ежедневно, суммарной продолжительностью не менее 150 минут в неделю
- коррекция диеты:
- рекомендуется максимальное ограничение жиров (прежде всего животного происхождения) и сахаров; умеренное (в размере \
половины привычной порции) – продуктов, состоящих преимущественно из сложных углеводов (крахмалов) и белков; \
неограниченное потребление продуктов с минимальной калорийностью (в основном богатых водой и клетчаткой овощей);
- рекомендуется потребление углеводов в составе овощей, цельнозерновых, молочных продуктов, в противовес другим \
источникам углеводов, содержащих дополнительно насыщенные или трансжиры, сахара или натрий;
- рекомендуется включать в рацион продукты, богатые моно- и полиненасыщенными жирными кислотами (рыба, растительные \
масла);
- не рекомендуется употребление алкогольных напитков;
"""
        rec_common = """Общепринятая прегравидарная подготовка согласно клиническому протоколу МАРС:
...
"""
        rec_stress = """Стресс-менеджмент: 
- управление стрессом с помощью когнитивно-поведенческого подхода;
- управление стрессом и релаксация с помощью таких методов, как йогатерапия; 
- снижение нагрузки на работе; 
- снижение психосоциального стресса; 
- оказание эмоциональной поддержки матерям с синдромом потери плода.
"""
        rec_mf_off_label = """Рассмотреть возможность назначения Метформина off-label.
Суточная доза – 1000 мг после или во время приема пищи, разделенная на 2 приема (при отсутствии противопоказаний);
"""
        if (self.label_result.text() == "Беременная высокого риска преэклампсии") and \
                (self.label_result2.text() != "У пациентки признаков предиабета нет"):
            self.scroll_label.setText(rec_main + '1. ' + rec_met + '2. ' + rec_lifestyle + '3. ' +
                                      rec_stress + '4.' + rec_common)
        elif (self.label_result.text() == "Беременная высокого риска преэклампсии") and \
                (self.label_result2.text() == "У пациентки признаков предиабета нет"):
            self.scroll_label.setText(rec_main + '1. ' + rec_mf_off_label + '2. ' + rec_lifestyle + '3. ' +
                                      rec_stress + '4.' + rec_common)
        elif (self.label_result.text() == "Беременная среднего риска преэклампсии") and \
                (self.label_result2.text() != "У пациентки признаков предиабета нет"):
            self.scroll_label.setText(rec_main + '1. ' + rec_met + '2. ' + rec_lifestyle + '3. ' + rec_common)
        elif (self.label_result.text() == "Беременная среднего риска преэклампсии") and \
                (self.label_result2.text() == "У пациентки признаков предиабета нет"):
            self.scroll_label.setText(rec_main + '1. ' + rec_lifestyle + '2. ' + rec_common)
        elif self.label_result.text() == "Беременная низкого риска преэклампсии":
            self.scroll_label.setText(rec_main + '1. ' + rec_common)
        else:
